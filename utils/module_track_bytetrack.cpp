    ...
    for(auto &&clsboxes: clss_bboxes){//loop class
        ...
        for(auto &&box: clsboxes.second){//loop boxes for each class
            ...
            for(auto &&trackOUT: vecTrackOUT){
                if(trackOUT.detect_idx >= clsboxes.second.size()) continue;
...
// void ByteTrackor::add_trackor(int cam_uuid){
//     if(m_trackors.find(cam_uuid)==m_trackors.end()){
//         //没有找到，新建一个
//         BYTETracker_Ptr m_trackor = std::make_shared<BYTETracker>(m_track_threshold, m_high_detect_threshold, m_fps,m_nn_buf);
//         m_trackors[cam_uuid] = m_trackor;
//     }
// }

// void ByteTrackor::update(TvaiImage &tvimage, VecObjBBox &bboxIN){
//     int cam_uuid = tvimage.uuid_cam;
//     add_trackor(cam_uuid);
//     vector<Object> vecObjIN;
//     for(auto &&box: bboxIN){
//         Object _box;
//         _box.rect.x = box.rect.x;
//         _box.rect.y = box.rect.y;
...
// void ByteTrackor::update(VecObjBBox &bboxIN, int cam_uuid){
//     add_trackor(cam_uuid);
//     vector<Object> vecObjIN;
//     for(auto &&box: bboxIN){
//         Object _box;
