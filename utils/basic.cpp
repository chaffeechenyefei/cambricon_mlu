...
cv::Mat resize_aspect(cv::Mat &Input, cv::Size OupSz, bool pad_both_side, float &aspect_ratio){
    ...
    cv::Mat Output = cv::Mat::zeros(OupSz, CV_8UC3);
    cv::Mat resizedInput;
    cv::resize(Input, resizedInput, cv::Size(_w, _h));
    int offset_x=0, offset_y=0;
    if(pad_both_side){
        offset_x = (OupW - _w)/2;
        offset_y = (OupH - _h)/2;
    }
    resizedInput.copyTo(Output(cv::Rect(offset_x,offset_y, _w,_h)));
    // if(inpRGB){
    //     if(oupRGB)
